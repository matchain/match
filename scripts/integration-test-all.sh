...
## default port prefixes for matchd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/matchd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/matchd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/matchd add-genesis-account \
    "$("$PWD"/build/matchd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000amatch,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/matchd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/matchd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/matchd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/matchd start --pruning=nothing --rpc.unsafe \
    ...
    EVMOS_PID=$!
    echo "started match node, pid=$EVMOS_PID"
    # add PID to array
    arr+=("$EVMOS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test match node $HOST_RPC ..."
