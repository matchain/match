...
func init() {
	cfg := sdk.GetConfig()
	cfg.SetBech32PrefixForAccount("match", "matchpub")
}
...
func TestGetMatchAddressFromBech32(t *testing.T) {
	testCases := []struct {
		name       string
		address    string
		expAddress string
		...
		{
			"match address",
			"match1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			"match1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			false,
		},
		{
	...
	for _, tc := range testCases {
		addr, err := GetMatchAddressFromBech32(tc.address)
		if tc.expError {
...
func TestMatchCoinDenom(t *testing.T) {
	testCases := []struct {
		name     string
		denom    string
		expError bool
