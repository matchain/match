...
const (
	// MainnetChainID defines the Match EIP155 chain ID for mainnet
	MainnetChainID = "match_698"
	// TestnetChainID defines the Match EIP155 chain ID for testnet
	TestnetChainID = "match_699"
	// BaseDenom defines the Match mainnet denomination
	BaseDenom = "match"
)

// IsMainnet returns true if the chain-id has the Match mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Match testnet EIP155 chain prefix.
...
// GetMatchAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `match`).
// The function fails if the provided bech32 address is invalid.
func GetMatchAddressFromBech32(address string) (sdk.AccAddress, error) {
