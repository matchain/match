...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(matchtypes.AttoMatch, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(matchtypes.AttoMatch, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(matchtypes.AttoMatch)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, matchtypes.AttoMatch, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
