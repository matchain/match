...
// AppName defines the Ledger app used for signing. Match uses the Ethereum app
const AppName = "Ethereum"

var (
	// SupportedAlgorithms defines the list of signing algorithms used on Match:
	//  - eth_secp256k1 (Ethereum)
	SupportedAlgorithms = keyring.SigningAlgoList{hd.EthSecp256k1}
	// SupportedAlgorithmsLedger defines the list of signing algorithms used on Match for the Ledger device:
...
func Option() keyring.Option {
	return func(options *keyring.Options) {
		...
		//options.LedgerDerivation = func() (cosmosLedger.SECP256K1, error) { panic(errors.New("todo support")) }
	}
}
