...
import (
	...
	cryptocodec "github.com/matchain/match/crypto/codec"
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("match", keyring.BackendTest, dir, mockIn, TestCodec, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, matchtypes.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, matchtypes.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := hdwallet.MustParseDerivationPath(matchtypes.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Match implementation
	...
	// Equality of Eth and Match implementation
	...
	// Inequality of wrong derivation path of Eth and Match implementation
