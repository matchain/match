...
func (k *Keeper) RefundGas(ctx sdk.Context, msg core.Message, leftoverGas uint64, denom string) error {
	...
	//used := new(big.Int).Mul(new(big.Int).SetUint64(msg.Gas()-leftoverGas), msg.GasPrice())
	...
	if msg.To() != nil {
		...
		if acct.IsContract() {
			err := k.rewardKeeper.Send(ctx, msg, usedGas, denom)
			if err != nil {
				return errorsmod.Wrapf(err, "failed to send reward used gas %d", usedGas)
			}
		}
	}

	err := k.rewardKeeper.Burn(ctx, msg, usedGas, denom)
	if err != nil {
		return errorsmod.Wrapf(err, "failed to burn used gas %d", usedGas)
	}
