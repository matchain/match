...
Thank you for considering making contributions to Match and related repositories!

Match uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
1. Either [open](https://github.com/matchain/match/issues/new/choose)
   or [find](https://github.com/matchain/match/issues) an issue you have identified and would like to contribute to
   resolving.
2. Participate in thoughtful discussion on that issue.
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Match team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the match repo, to ensure a
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Match repository
...
Check out our [good first issues](https://github.com/match/match/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Match,
please create an [ADR](https://github.com/match/match/blob/main/docs/architecture/README.md)
...
While my fork lives at `https://github.com/match/match`,
the code should never exist at `$GOPATH/src/github.com/match/match`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/match/match`,
...
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/match/match`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:tharsis/match.git`

Now `origin` refers to my fork and `upstream` refers to the match version.
So I can `git push -u origin master` to update my fork,
and make pull requests to match from there.
...
with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Match.
...
4. Create your feature branch from `development` either on `github.com/match/match`, or your fork (
   using `git remote add origin`).
5. Before submitting a pull request, begin `git rebase` on top of `development`.

## <span id="testing">Testing</span>

Match uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Match repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Match utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Match team currently cuts releases on a need to have basis.
