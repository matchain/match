...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("match_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
		BondDenom:         "amatch",
		MinGasPrices:      fmt.Sprintf("0.000006%s", matchtypes.AttoMatch),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, matchtypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, matchtypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, matchtypes.PowerReduction),
...
// NewAppConstructor returns a new Match AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !matchtypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "matchd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "matchcli")
		...
		customAppTemplate, _ := config.AppConfig(matchtypes.AttoMatch)
