...
import (
	...
	evm "github.com/matchain/match/x/evm/types"
)
...
func DeployContract(
	...
	chainID := matchApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := matchApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: matchApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(matchApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, matchApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := matchApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := matchApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := matchApp.EvmKeeper.GetNonce(ctx, from)
