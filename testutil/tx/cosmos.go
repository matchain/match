...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdk.NewIntFromUint64(uint64(feeAmt))) // 0.01 MATCH
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'match_699-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appMatch.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appMatch.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
