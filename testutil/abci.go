...
import (
	...
	"github.com/matchain/match/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Match, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func DeliverTx(
	...
	return BroadcastTxBytes(appMatch, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appMatch, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	return BroadcastTxBytes(appMatch, txConfig.TxEncoder(), tx)
...
func CheckTx(
	...
	return checkTxBytes(appMatch, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Match, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func checkTxBytes(app *app.Match, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
