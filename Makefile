...
HTTPS_GIT := https://github.com/matchain/match.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=match \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(MATCH_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm match || true
	# create a new container from the latest image
	$(DOCKER) create --name match -t -i ${DOCKER_IMAGE}:latest match
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp match:/usr/bin/matchd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/matchain/match"
	godoc -http=:6060
...
protoCosmosImage=$(DOCKER) run --network host --rm -v $(CURDIR):/workspace --workdir /workspace -u root $(protoCosmosName)
# ------
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(MATCH_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/match:Z match/node "./matchd testnet init-files --v 4 -o /match --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\matchd:/match\Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)\build\node1\matchd:/match\Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)\build\node2\matchd:/match\Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)\build\node3\matchd:/match\Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
else
	@docker run --rm -v $(CURDIR)/build/node0/matchd:/match:Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)/build/node1/matchd:/match:Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)/build/node2/matchd:/match:Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
	@docker run --rm -v $(CURDIR)/build/node3/matchd:/match:Z match/node "./matchd tendermint unsafe-reset-all --home=/match"
endif
...
PACKAGE_NAME:=github.com/matchain/match
